...
const unsigned long sendInterval = 1000; // Interval between sending commands (500ms)
...
void loop() {
  ...
  if (digitalRead(buttonPin) == LOW && !connectionActive && disconnectedFlag) {
  ...
  if ((digitalRead(buttonPin2) == LOW)) {  // Check if the reset button is pressed
